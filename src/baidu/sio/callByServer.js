...
baidu.sio.callByServer = function(url, callback, opt_options) {
    ...
    callbackImpl = baidu.lang.isFunction(callback) ? callback : window[callback];

    // 不管callback是string还是function，都重写callback
    callbackName = prefix + Math.floor(Math.random() * 2147483648).toString(36);
    window[callbackName] = function() {
        try {
            if(baidu.lang.isFunction(callbackImpl)){
                callbackImpl.apply(window, arguments);
            }
