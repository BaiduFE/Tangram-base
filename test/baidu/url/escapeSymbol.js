...
test("确认除此外的其他字符不需要转义", function() {
	...
	$.ajax( {
		...
		success : function(data) {
			equals(data, "!\"$'()*,-.0123456789:;<>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~");
			start();
		}
	});
	// },
...
// describe(
// "baidu.url.escapeSymbol测试",
// {
// "字符串只含有转义的七个字符" : function() {
// value_of(baidu.url.escapeSymbol("%")).should_be("%25");
// value_of(baidu.url.escapeSymbol("&")).should_be("%26");
// value_of(baidu.url.escapeSymbol("+")).should_be("%2B");
// value_of(baidu.url.escapeSymbol("/")).should_be("%2F");
// value_of(baidu.url.escapeSymbol("#")).should_be("%23");
// value_of(baidu.url.escapeSymbol("=")).should_be("%3D");
// value_of(baidu.url.escapeSymbol(" ")).should_be("%20");
// },
// "含有转义的七个字符的字符串正常转义" : function() {
// value_of(
// baidu.url
// .escapeSymbol("aQ1!%sW2@&dE3$+fR4^/gT5*#hY6(=jU7) "))
// .should_be(
// "aQ1!%25sW2@%26dE3$%2BfR4^%2FgT5*%23hY6(%3DjU7)%20");
// value_of(baidu.url.escapeSymbol("这%是&中文+吗/好像有点#像=你说 呢"))
// .should_be("这%25是%26中文%2B吗%2F好像有点%23像%3D你说%20呢");
// value_of(
// baidu.url
// .escapeSymbol("％%＆&＋+、/＃#＝= ａＡｑＷ%\t&\a+\b/\f#\?=\r \%\&\+\/\#\=\ "))
// .should_be(
...
// },
// "没有这七个字符" : function() {
// value_of(baidu.url.escapeSymbol("aQ1!sW2@dE3$fR4^gT5*hY6(jU7)"))
// .should_be("aQ1!sW2@dE3$fR4^gT5*hY6(jU7)");
// value_of(baidu.url.escapeSymbol("这是中文吗好像有点像你说呢")).should_be(
// "这是中文吗好像有点像你说呢");
// value_of(baidu.url.escapeSymbol("％＆＋、＃＝ ａＡｑＷ\t\a\b\f\?\r"))
// .should_be("％＆＋、＃＝ ａＡｑＷ\t\a\b\f\?\r");
// },
// '确认除此外的其他字符不需要转义' : function() {
// var _url, text, xhr, url = 'url/escapeSymbol.php?';
// var urlparms = [], url1 = [];
// var list = [ '20', '23', '25', '26', '2b', '2f', '3d' ];
// var _listchar = ' ' + list.join(' ') + ' ';
// for ( var i = 32; i < 127; i++) {
...
// }
// _url = url + urlparms.join('');
// xhr = baidu.ajax.request(_url, {
// 'onsuccess' : function(xhr) {
// text = xhr.responseText;
// },
// 'async' : false
// });
// value_of(text).should_be(urlparms.join('')+'&');
// }
// });
