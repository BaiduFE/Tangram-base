...
//#JSCOVERAGE_IF /MSIE/.test(navigator.userAgent)
  ...
  */
  ...
  summaryDiv.style.height = (viewportHeight - jscoverage_findPos(summaryDiv) - 21) + 'px';//设置summary的height
...
function jscoverage_createLink(file, line) {
  ...
  link.href = '#';
  link.onclick = jscoverage_createHandler(file, line);//此方法用于生成代码行显示
...
function jscoverage_recalculateSummaryTab(cc) {//生成summary数据的主要方法
...
//  document.writeln(tbody.innerHTML);
  var caseliststr = tbody.innerHTML;
  jscoverage_endLengthyOperation();
  return caseliststr;
}
...
function jscoverage_checkbox_click() {//选中checkbox后，计算行宽度和高度
...
//  var checkbox = document.getElementById('checkbox');
  var showMissingColumn = true;
  setTimeout(function() {
    if (showMissingColumn) {//改成静态html时，应该默认执行复选框被选中时的操作，即默认执行下面语句
      jscoverage_appendMissingColumn();
    }
    else {
...
function jscoverage_recalculateSourceTab() {
  ...
  if(jscoverage_currentFilecontent){
     var sourceDiv = document.getElementById('sourceDiv');
     sourceDiv.innerHTML = jscoverage_currentFilecontent;
  }
  jscoverage_scrollToLine();//此处把jscoverage_scrollToLine 从jscoverage_makeTable 中摘取出来
  
//  setTimeout(jscoverage_makeTable, 0);//此行取消，因为source html 在run.js里生成了
}
...
function jscoverage_tab_click(e) {//如果静态页面，应该放弃选summary tab的操作，直接选复选框
  if (jscoverage_inLengthyOperation) {
    return;
  }
  var target;
