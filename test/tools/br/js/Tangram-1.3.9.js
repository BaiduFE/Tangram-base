(function() {
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/ajax.js author: allstar, erik version: 1.1.0 date: 2009/11/13
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/fn.js author: berg version: 1.0.0 date: 2010/11/02
	 */
	...
	baidu.fn = baidu.fn || {};
	/*
	 * Tangram Copyright 2011 Baidu Inc. All rights reserved.
	 */
	 ...
	 * @config {Function} [onsuccess] 请求成功时触发，function(XMLHttpRequest xhr,
	 *         string responseText)。
	 * @config {Function} [onfailure] 请求失败时触发，function(XMLHttpRequest xhr)。
	 * @config {Function} [onbeforerequest] 发送请求之前触发，function(XMLHttpRequest
	 *         xhr)。
	 * @config {Function} [on{STATUS_CODE}]
	 *         当请求为相应状态码时触发的事件，如on302、on404、on500，function(XMLHttpRequest
	 *         xhr)。3XX的状态码浏览器无法获取，4xx的，可能因为未知问题导致获取失败。
	...
	baidu.ajax.request = function(url, options) {
		options = options || {};
		var data = options.data || "", async = !(options.async === false), username = options.username
				|| "", password = options.password || "", method = (options.method || "GET")
				.toUpperCase(), headers = options.headers || {},
		// 基本的逻辑来自lili同学提供的patch
		timeout = options.timeout || 0, eventHandlers = {}, tick, key, xhr;
		...
		function stateChangeHandler() {
			if (xhr.readyState == 4) {
				...
				/*
				 * NOTE: Testing discovered that for some bizarre reason, on
				 * Mozilla, the JavaScript <code>XmlHttpRequest.onreadystatechange</code>
				 * handler function maybe still be called after it is deleted.
				 * The theory is that the callback is cached somewhere. Setting
				 * it to null or an empty function does seem to work properly,
				 * though.
				 * 
				 * On IE, there are two problems: Setting onreadystatechange to
				 * null (as opposed to an empty function) sometimes throws an
				 * exception. With particular (rare) versions of jscript.dll,
				 * setting onreadystatechange from within onreadystatechange
				 * causes a crash. Setting it from within a timeout fixes this
				 * bug (see issue 1610).
				 * 
				 * End result: *always* set onreadystatechange to an empty
				 * function (never to null). Never set onreadystatechange from
				 * within onreadystatechange (always in a setTimeout()).
				 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/ajax/form.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * @config {String} [password] 密码
	 * @config {Object} [headers] 要设置的http request header
	 * @config {Function} [replacer] 对参数值特殊处理的函数,replacer(string value, string
	 *         key)
	 * @config {Function} [onbeforerequest] 发送请求之前触发，function(XMLHttpRequest
	 *         xhr)。
	 * @config {Function} [onsuccess] 请求成功时触发，function(XMLHttpRequest xhr,
	...
	baidu.ajax.form = function(form, options) {
		options = options || {};
		var elements = form.elements, len = elements.length, method = form
				.getAttribute('method'), url = form.getAttribute('action'), replacer = options.replacer
				|| function(value, name) {
					return value;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/ajax/get.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 *            url 发送请求的url地址
	 * @param {Function}
	 *            [onsuccess] 请求成功之后的回调函数，function(XMLHttpRequest xhr, string
	 *            responseText)
	 * @meta standard
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/ajax/post.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/indexOf.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/each.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 *            source 需要遍历的数组
	 * @param {Function}
	 *            iterator 对每个数组元素进行调用的函数，该函数有两个参数，第一个为数组元素，第二个为数组索引值，function
	 *            (item, index)。
	 * @param {Object}
	 *            [thisObject] 函数调用时的this指针，如果没有此参数，默认是当前遍历的数组
	 * @remark each方法不支持对Object的遍历,对Object的遍历使用baidu.object.each 。
	 * @shortcut each
	 * @meta standard
	 * 
	 * @returns {Array} 遍历的数组
	...
	baidu.each = baidu.array.forEach = baidu.array.each = function(source,
		...
		if ('function' == typeof iterator) {
			for (i = 0; i < len; i++) {
				...
				// used as the this for each invocation of the callback. If it
				// is not provided, or is null, the global object associated
	...
	baidu.array.every = function(source, iterator, thisObject) {
		...
		for (; i < len; i++) {
			if (i in source
					&& !iterator.call(thisObject || source, source[i], i)) {
				return false;
			}
		}
		return true;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/find.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 *            keys 作为key的数组
	 * @param {Array}
	 *            [values] 作为value的数组，未指定此参数时，默认值将对象的值都设为true。
	 * 
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/lastIndexOf.js author: erik version: 1.1.0 date:
	 * 2009/11/14
	 */
	...
	baidu.array.map = function(source, iterator, thisObject) {
		var results = [], i = 0, l = source.length;
		for (; i < l; i++) {
	 ...
	 * @param {Function}
	 *            iterator
	 *            对每个数组元素进行处理的函数，函数接受四个参数：上一次reduce的结果（或初始值），当前元素值，索引值，整个数组.
	 * @param {Object}
	 *            [initializer] 合并的初始项，如果没有此参数，默认用数组中的第一个值作为初始值.
	 * @return {Array} reduce后的值.
	 * @version 1.3.4
	 * @see baidu.array.reduce
	 */
	baidu.array.reduce = function(source, iterator, initializer) {
		var i = 0, l = source.length, found = 0;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/remove.js author: erik version: 1.1.0 date: 2009/11/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/removeAt.js author: erik version: 1.1.0 date:
	 * 2009/11/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/array/unique.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/chrome.js author: allstar version: 1.1.0 date:
	 * 2009/11/24
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/firefox.js author: allstar version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/ie.js author: allstar version: 1.1.0 date: 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/isGecko.js author: allstar version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/isStrict.js author: allstar version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/isWebkit.js author: allstar version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/maxthon.js author: allstar version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/opera.js author: allstar version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/browser/safari.js author: allstar, rocy version: 1.1.0 date:
	 * 2009/11/23
	 */
	...
	(function() {
		...
		/*
		 * 兼容浏览器为safari或ipad,其中,一段典型的ipad UA 如下: Mozilla/5.0(iPad; U; CPU iPhone
		 * OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 (KHTML, like
		 * Gecko) Version/4.0.4 Mobile/7B314 Safari/531.21.10
		 */
		if (/(\d+\.\d)?(?:\.\d)?\s+safari\/?(\d+\.\d+)?/i.test(ua)
				&& !/chrome/i.test(ua)) {
			/**
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie/_isValidKey.js author: erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie/getRaw.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie/get.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie/setRaw.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie/remove.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/cookie/set.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/date.js author: erik version: 1.1.0 date: 2009/12/04
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/number.js author: erik version: 1.1.0 date: 2009/12/2
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/number/pad.js author: dron, erik, berg version: 1.1.0 date:
	 * 20100412
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/date/format.js author: erik version: 1.1.0 date: 2009/12/04
	 */
	...
	baidu.date.format = function(source, pattern) {
		...
		var pad = baidu.number.pad, year = source.getFullYear(), month = source
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/date/parse.js author: erik version: 1.1.0 date: 2009/12/04
	 */
	 ...
	 * 被解释为七月而不是六月。 </li>
	 * <li>在所提供的日期中，如果所指定的星期几的值与按照该日期中剩余部分所确定的星期几的值不符合，那么该指定值就会被忽略。例如，尽管 1996 年
	 * 11 月 9 日实际上是星期五，"Tuesday November 9 1996" 也还是可以被接受并进行解析的。但是结果 date
	 * 对象中包含的是 "Friday November 9 1996"。 </li>
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom.js author: allstar, erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_NAME_ATTRS.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/2
	 */

	/**
	 * 提供给setAttr与getAttr方法作名称转换使用 ie6,7下class要转换成className
	 * 
	 * @meta standard
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isString.js author: erik version: 1.1.0 date: 2009/12/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/g.js author: allstar, erik, berg version: 1.3 date:
	 * 2010-07-07
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/g.js author: allstar, erik version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_matchNode.js author: allstar version: 1.1.0 date:
	 * 2009/11/18
	 */
	 ...
	 * @param {string}
	 *            direction 遍历的方向名称，取值为previousSibling,nextSibling
	 * @param {string}
	 *            start
	 *            遍历的开始位置，取值为firstChild,lastChild,previousSibling,nextSibling
	 * @meta standard
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFilters.js author: allstar version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFilter/color.js author: allstar, erik version:
	 * 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFilter/filter.js author: allstar, erik version:
	 * 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFilter/px.js author: allstar, erik version: 1.1.0
	 * date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFixer.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFixer/display.js author: allstar version: 1.1.0
	 * date: 2009/11/24
	 */
	 ...
	 * @meta standard
	 */
	baidu.dom._styleFixer.display = baidu.browser.ie && baidu.browser.ie < 8 ? { // berg:
																					// 修改到<8，因为ie7同样存在这个问题，from
																					// 先伟
		set : function(element, value) {
			...
			: baidu.browser.firefox && baidu.browser.firefox < 3 ? {
				set : function(element, value) {
					element.style.display = value == 'inline-block' ? '-moz-inline-box'
							: value;
				}
			}
					: null;
	/*
	 * Tangram Copyright 2009 Baidu Inc. All right reserved.
	 * 
	 * path: baidu/dom/_styleFixer/float.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFixer/opacity.js author: allstar version: 1.1.0
	 * date: 2009/11/17
	 */
	...
	baidu.dom._styleFixer.opacity = baidu.browser.ie ? {
		get : function(element) {
			...
			return filter && filter.indexOf("opacity=") >= 0 ? (parseFloat(filter
					.match(/opacity=([^)]*)/)[1]) / 100)
					+ ""
					: "1";
		},
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getDocument.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/toCamelCase.js author: erik, berg version: 1.2 date:
	 * 2010-06-22
	 */
	...
	baidu.dom.getStyle = function(element, key) {
		...
		if (!value) {
			...
			if (fixer) {
				value = fixer.get ? fixer.get(element) : baidu.dom.getStyle(
						element, fixer);
			}
		}
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/_styleFixer/textOverflow.js author: allstar version:
	 * 1.1.0 date: 2009/11/17
	 */

	/**
	 * 提供给setStyle与getStyle使用，在做textOverflow时会向element对象中添加,_baiduOverflow,
	 * _baiduHTML两个属性保存原始的innerHTML信息
	 */
	baidu.dom._styleFixer.textOverflow = (function() {
		...
		function count(element, width, ellipsis) {
			/* 计算cache的名称 */
			var o = baidu.browser.ie ? element.currentStyle || element.style
					: getComputedStyle(element, null), fontWeight = o.fontWeight, cacheName = "font-family:"
					+ o.fontFamily
					+ ";font-size:"
					+ o.fontSize
					+ ";word-spacing:"
					+ o.wordSpacing
					+ ";font-weight:"
					+ ((parseInt(fontWeight) || 0) == 401 ? 700 : fontWeight)
					+ ";font-style:"
					+ o.fontStyle
					+ ";font-variant:"
					+ o.fontVariant, cache = fontSizeCache[cacheName];
			...
			if (!cache) {
				...
				/* 计算ASCII字符的宽度cache */
				for ( var i = 0; i < 256; i++) {
					i == 32 ? (o.innerHTML = "&nbsp;") : setText(o, String
							.fromCharCode(i));
			...
			var node = element.firstChild, charWidth = cache[256], wordSpacing = cache[257], ellipsisWidth = cache[258], wordWidth = [], ellipsis = ellipsis ? ellipsisWidth
					: 0; node; node = node.nextSibling) {
				if (width < ellipsis) {
					...
					for ( var i = 0, text = node.nodeValue, length = text.length; i < length; i++) {
						...
						width -= (i ? wordSpacing : 0)
								+ (o < 256 ? cache[o] : charWidth);
						if (width < ellipsis) {
							break;
						}
					}
		...
		return {
			get : function(element) {
				var browser = baidu.browser, getStyle = dom.getStyle;
				return (browser.opera ? getStyle("OTextOverflow")
						: browser.firefox ? element._baiduOverflow
			...
			set : function(element, value) {
				...
				if (element.tagName == "TD" || element.tagName == "TH"
						|| browser.firefox) {
					element._baiduHTML
							&& (element.innerHTML = element._baiduHTML);

					if (value == "ellipsis") {
						element._baiduHTML = element.innerHTML;
						var o = document.createElement("div"), width = element
								.appendChild(o).offsetWidth;
						element.removeChild(o);
						count(element, width);
					} else {
						element._baiduHTML = "";
				...
				o = element.style;
				browser.opera ? (o.OTextOverflow = value)
						: browser.firefox ? (element._baiduOverflow = value)
								: (o.textOverflow = value);
			}
		};
	})();
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/trim.js author: dron, erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All right reserved.
	 * 
	 * path: baidu/dom/addClass.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/2
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {string}
	 *            className 要添加的className，允许同时添加多个class，中间使用空白符分隔
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/children.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/contains.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * @param {HTMLElement|string}
	 *            container 包含元素或元素的id
	 * @param {HTMLElement|string}
	 *            contained 被包含元素或元素的id
	 * @meta standard
	...
	baidu.dom.contains = function(container, contained) {
		...
		return container.contains ? container != contained
				&& container.contains(contained) : !!(container
				.compareDocumentPosition(contained) & 16);
	};
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/setAttr.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/setAttrs.js author: allstar version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 *            tagName 标签名称.
	 * @param {Object}
	 *            opt_attributes 元素创建时拥有的属性，如style和className.
	 * @version 1.3
	 * @meta standard
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/guid.js author: meizz version: 1.1.0 date: 2010/02/04
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/_instances.js author: meizz, erik version: 1.1.0 date:
	 * 2009/12/1
	 */

	/**
	 * 所有类的实例的容器 key为每个实例的guid
	 * 
	 * @meta standard
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isFunction.js author: rocy version: 1.1.2 date:
	 * 2010/06/12
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/Class.js author: meizz, erik version: 1.1.0 date:
	 * 2009/12/1
	 */
	 ...
	 * 
	 * @name dispose
	 * @grammar obj.dispose() TODO: 将_listeners中绑定的事件剔除掉
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/Event.js author: meizz, erik, berg version: 1.1.1 date:
	 * 2009/11/24 modify: 2010/04/19 berg
	 */
	 ...
	 * @param {string}
	 *            type 事件类型名称。为了方便区分事件和一个普通的方法，事件类型名称必须以"on"(小写)开头。
	 * @param {Object}
	 *            [target]触发事件的对象
	 * @meta standard
	 ...
	 *            type 自定义事件的名称
	 * @param {Function}
	 *            handler 自定义事件被触发时应该调用的回调函数
	 * @param {string}
	 *            [key] 为事件监听函数指定的名称，可在移除时使用。如果不提供，方法会默认为它生成一个全局唯一的key。
	 ...
	 * @param {baidu.lang.Event|String}
	 *            event Event对象，或事件名称(1.1.1起支持)
	 * @param {Object}
	 *            options 扩展参数,所含属性键值会扩展到Event对象上(1.2起支持)
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/createSingle.js author: meizz, berg version: 1.1.2 date:
	 * 2010-05-13
	 */
	...
	/*
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/dragManager.js author: rocy version: 1.4.0 date:
	 * 2010/10/14
	 */
	 ...
	 *            element 被拖曳的元素
	 * @param {JSON}
	 *            options 拖曳配置项 {toggle, autoStop, interval, capture, range,
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/_listeners.js author: erik version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/on.js author: erik version: 1.1.0 date: 2009/12/16
	 */
	 ...
	 * @param {HTMLElement|string|window}
	 *            element 目标元素或目标元素id
	 * @param {string}
	 *            type 事件类型
	 * @param {Function}
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/un.js author: erik version: 1.1.0 date: 2009/12/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/preventDefault.js author: erik version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/object.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page.js author: erik version: 1.1.0 date: 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getScrollTop.js author: erik version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getScrollLeft.js author: erik version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getMousePosition.js author: meizz version: 1.1.0 date:
	 * 2010/06/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getPosition.js author: berg version: 1.2.0 date:
	 * 2010/12/16
	 * 
	...
	baidu.dom.getPosition = function(element) {
		...
		BUGGY_GECKO_BOX_OBJECT = browser.isGecko > 0 && doc.getBoxObjectFor
			...
			"left" : 0,
			"top" : 0
		}, viewport = (browser.ie && !browser.isStrict) ? doc.body
		...
		if (element.getBoundingClientRect) { // IE and Gecko 1.9+
			...
			htmlBorderLeftWidth = parseInt(getStyle(htmlDom, 'borderLeftWidth')), htmlBorderTopWidth = parseInt(getStyle(
					htmlDom, 'borderTopWidth'));
			if (browser.ie && !browser.isStrict) {
				pos.left -= isNaN(htmlBorderLeftWidth) ? 2
						: htmlBorderLeftWidth;
			 ...
			 * (doc.getBoxObjectFor && !BUGGY_GECKO_BOX_OBJECT){ // gecko 1.9-
			...
			// 对body offsetTop的修正
			if (browser.opera > 0
					|| (browser.isWebkit > 0 && getStyle(element, 'position') == 'absolute')) {
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/drag.js author: meizz, berg, lxp version: 1.1.0 date:
	 * 2010/06/02
	 */
	 ...
	 * @param {Array}
	 *            options.range
	 *            限制drag的拖拽范围，数组中必须包含四个值，分别是上、右、下、左边缘相对上方或左方的像素距离。默认无限制
	 * @param {Number}
	 *            options.interval 拖曳行为的触发频度（时间：毫秒）
	 * @param {Boolean}
	 *            options.capture 鼠标拖曳粘滞
	 * @param {Object}
	 *            options.mouseEvent
	 *            键名为clientX和clientY的object，若不设置此项，默认会获取当前鼠标位置
	 * @param {Function}
	 *            options.ondragstart drag开始时触发
	 * @param {Function}
	 *            options.ondrag drag进行中触发
	 * @param {Function}
	 *            options.ondragend drag结束时触发
	 * @param {function}
	 *            options.autoStop 是否在onmouseup时自动停止拖拽。默认为true
	 * @version 1.2
	 * @remark
	 * 
	 ...
	 *            element 被拖曳的元素
	 * @param {JSON}
	 *            options 拖曳配置项 {autoStop, interval, capture, range,
	 *            ondragstart, ondragend, ondrag, mouseEvent}
	 */
	(function() {
		var target, // 被拖曳的DOM元素
		op, ox, oy, // timer,
		top, left, mozUserSelect, lastLeft, lastTop, isFunction = baidu.lang.isFunction, timer, offset_parent, offset_target;
		...
		baidu.dom.drag = function(element, options) {
			...
			if (baidu.getStyle(target, 'position') == "absolute") {
				top = offset_target.top
						- (target.offsetParent == document.body ? 0
								: offset_parent.top);
				left = offset_target.left
						- (target.offsetParent == document.body ? 0
								: offset_parent.left);
			} else {
				top = parseFloat(baidu.getStyle(target, "top"))
						|| -parseFloat(baidu.getStyle(target, "bottom")) || 0;
				left = parseFloat(baidu.getStyle(target, "left"))
						|| -parseFloat(baidu.getStyle(target, "right")) || 0;
			}
		...
		function render(e) {
			var rg = op.range, xy = baidu.page.getMousePosition(), el = left
					+ xy.x - ox, et = top + xy.y - oy;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/setStyle.js author: allstar version: 1.1.0 date:
	 * 2009/11/18
	 */
	...
	baidu.dom.setStyle = function(element, key, value) {
		...
		(fixer && fixer.set) ? fixer.set(element, value) : (element.style[fixer
				|| key] = value);
	 ...
	 * @config {Function} [toggle]
	 *         在每次ondrag的时候，会调用这个方法判断是否应该停止拖拽。如果此函数返回值为false，则停止拖拽.
	 * @version 1.2
	 * @remark 要拖拽的元素必须事先设定样式的postion值，如果postion为absloute，并且没有设定top和left，拖拽开始时，无法取得元素的top和left值，这时会从[0,0]点开始拖拽<br>
	...
	baidu.dom.draggable = function(element, options) {
		...
		var manager, events = [ 'ondragstart', 'ondrag', 'ondragend' ], i = events.length - 1, eventName, dragSingle, draggableSingle = {
			dispose : function() {
				dragSingle && dragSingle.stop();
				baidu.event
						.un(options.handler, 'onmousedown', handlerMouseDown);
		...
		if (element) {
			function handlerMouseDown(e) {
				...
				if (event.button > 1 // 只支持鼠标左键拖拽; 左键代码: IE为1,W3C为0
						...
						|| (baidu.lang.isFunction(options.toggle) && !options
								.toggle())) {
					return;
				}
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/intersect.js author: allstar version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * @param {HTMLElement|string}
	 *            element1 要检查的元素或元素的id
	 * @param {HTMLElement|string}
	 *            element2 要检查的元素或元素的id
	 * @see baidu.dom.contains
	 * 
	 * @returns {boolean} 两个元素是否相交的检查结果
	 */
	baidu.dom.intersect = function(element1, element2) {
		var g = baidu.dom.g, getPosition = baidu.dom.getPosition, max = Math.max, min = Math.min;

		element1 = g(element1);
		element2 = g(element2);

		var pos1 = getPosition(element1), pos2 = getPosition(element2);

		return max(pos1.left, pos2.left) <= min(pos1.left
				+ element1.offsetWidth, pos2.left + element2.offsetWidth)
				&& max(pos1.top, pos2.top) <= min(pos1.top
						+ element1.offsetHeight, pos2.top
						+ element2.offsetHeight);
	};
	/*
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/droppable.js author: rocy version: 1.4.0 date: 2010/10/14
	 */
	 ...
	 * @param {HTMLElement|string}
	 *            element 容器元素或者容器元素的ID
	 * @param {Object}
	 *            [options] 选项，拖拽元素对于容器元素的事件
	 * 
	...
	baidu.dom.droppable = function(element, options) {
		options = options || {};
		var manager = baidu.dom.ddManager, target = baidu.dom.g(element), guid = baidu.lang
				.guid(),
		// 拖拽进行时判断
		_dragging = function(event) {
			var _targetsDroppingOver = manager._targetsDroppingOver, eventData = {
				trigger : event.DOM,
				reciever : target
			};
			// 判断被拖拽元素和容器是否相撞
			if (baidu.dom.intersect(target, event.DOM)) {
				...
				if (!_targetsDroppingOver[guid]) {
					// 初次进入
					(typeof options.ondropover == 'function')
							&& options.ondropover.call(target, eventData);
				...
				// 出了容器区域
				if (_targetsDroppingOver[guid]) {
					(typeof options.ondropout == 'function')
							&& options.ondropout.call(target, eventData);
		...
		_dragend = function(event) {
			...
			if (baidu.dom.intersect(target, event.DOM)) {
				typeof options.ondrop == 'function'
						&& options.ondrop.call(target, eventData);
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/empty.js author: berg version: 1.0 date: 2010-07-06
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/first.js author: allstar, erik version: 1.1.0 date:
	 * 2009/11/18
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getAncestorBy.js author: allstar, erik version: 1.1.0
	 * date: 2009/12/02
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {Function}
	 *            method 判断祖先元素条件的函数，function (element)
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getAncestorByClass.js author: allstar, erik version:
	 * 1.1.0 date: 2009/12/02
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {string}
	 *            className 祖先元素的class，只支持单个class
	 * @remark 使用者应保证提供的className合法性，不应包含不合法字符，className合法字符参考：http://www.w3.org/TR/CSS2/syndata.html。
	...
	baidu.dom.getAncestorByClass = function(element, className) {
		element = baidu.dom.g(element);
		className = new RegExp("(^|\\s)" + baidu.string.trim(className)
				+ "(\\s|\x24)");
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getAncestorByTag.js author: allstar, erik version: 1.1.0
	 * date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getAttr.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All right reserved.
	 * 
	 * path: baidu/dom/getParent.js author: berg version: 1.0.0 date: 2010/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getText.js author: berg version: 1.0 date: 2010/07/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/getWindow.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/hasAttr.js author: berg version: 1.0 date: 2010/07/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/hasClass.js author: berg version: 1.0 date: 2010-07-06
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {string}
	 *            className 要判断的className，可以是用空格拼接的多个className
	 * @version 1.2
	 * @remark 对于参数className，支持空格分隔的多个className
	 * @see baidu.dom.addClass, baidu.dom.removeClass
	 * @meta standard
	 * @returns {Boolean}
	 *          是否拥有指定的className，如果要查询的classname有一个或多个不在元素的className中，返回false
	 */
	baidu.dom.hasClass = function(element, className) {
		...
		while (len--) {
			if (!(new RegExp("(^| )" + classArray[len] + "( |\x24)"))
					.test(className)) {
				return false;
			}
		}
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/hide.js author: allstar version: 1.1.0 date: 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/insertAfter.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/insertBefore.js author: allstar, erik version: 1.1.0
	 * date: 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/insertHTML.js author: allstar, erik,
	 * berg,wenyuxiang,lixiaopeng version: 1.1.2 date: 2010-07-13
	 */
	 ...
	 * @param {string}
	 *            position
	 *            插入html的位置信息，取值为beforeBegin,afterBegin,beforeEnd,afterEnd
	 * @param {string}
	 *            html 要插入的html
	 * @remark
	 * 
	 * 对于position参数，大小写不敏感<br>
	 * 参数的意思：beforeBegin&lt;span&gt;afterBegin this is span!
	 * beforeEnd&lt;/span&gt; afterEnd <br />
	...
	baidu.dom.insertHTML = function(element, position, html) {
		...
		if (element.insertAdjacentHTML) {
			...
			// 这里不做"undefined" != typeof(HTMLElement) &&
			// !window.opera判断，其它浏览器将出错？！
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/last.js author: allstar version: 1.1.0 date: 2009/11/18
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/next.js author: allstar version: 1.1.0 date: 2009/11/18
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/prev.js author: allstar version: 1.1.0 date: 2009/11/18
	 */
	...
	baidu.dom.prev = function(element) {
		return baidu.dom._matchNode(element, 'previousSibling',
				'previousSibling');
	};
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/escapeReg.js author: erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/q.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 * @param {string}
	 *            className 元素的class，只能指定单一的class，如果为空字符串或者纯空白的字符串，返回空数组。
	 * @param {string|HTMLElement}
	 *            [element] 开始搜索的元素，默认是document。
	 * @param {string}
	 *            [tagName] 要获取元素的标签名，如果没有值或者值为空字符串或者纯空白的字符串，表示不限制标签名。
	 ...
	 *            selector 选择器定义
	 * @param {HTMLElement |
	 *            DOMDocument} [context] 查找的上下文
	 * @param {Array}
	 *            [results] 查找的结果会追加到这个数组中
	 * @version 1.2
	 * @remark
	 * 
	 * 选择器支持所有的<a href="http://www.w3.org/TR/css3-selectors/">css3选择器</a>
	 * ，核心实现采用sizzle。可参考<a href="http://wiki.github.com/jeresig/sizzle/"
	 * target="_blank">sizzle 文档</a>
	 * 
					...
					if (!seed && !Sizzle.isXML(context)) {
						if (context.nodeType === 9) {
							...
							var old = context.getAttribute("id"), nid = old
									|| id, hasParent = context.parentNode, relativeHierarchySelector = /^\s*[+~]/
									.test(query);
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/remove.js author: allstar,berg version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * @param {HTMLElement|string}
	 *            element 需要移除的元素或元素的id
	 * @remark <b>注意：</b>对于移除的dom元素，IE下会释放该元素的空间，继续使用该元素的引用进行操作将会引发不可预料的问题。
	 * @meta standard
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/removeClass.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/02
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {string}
	 *            className 要移除的className，允许同时移除多个class，中间使用空白符分隔
	 * @remark 使用者应保证提供的className合法性，不应包含不合法字符，className合法字符参考：http://www.w3.org/TR/CSS2/syndata.html。
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/removeStyle.js author: wenyuxiang, berg version: 1.0.1
	 * date: 2010/9/10
	 */
	...
	baidu.dom.removeStyle = function() {
		var ele = document.createElement("DIV"), fn, _g = baidu.dom._g;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/object/each.js author: berg version: 1.1.1 date: 2010-04-19
	 */
	 ...
	 *            source 需要遍历的Object
	 * @param {Function}
	 *            iterator 对每个Object元素进行调用的函数，function (item, key)
	 * @version 1.1.1
	 * 
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/setStyles.js author: allstar version: 1.1.0 date:
	 * 2009/11/18
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/getTarget.js author: erik version: 1.1.0 date:
	 * 2009/11/17
	 */
	...
	/**
	 * 按照border-box模型设置元素的height和width值。只支持元素的padding/border/height/width使用同一种计量单位的情况。<br/>
	 * 不支持：<br/> 1. 非数字值(medium)<br/> 2. em/px在不同的属性中混用
	 * 
	 ...
	 */
	baidu.dom.setOuterHeight = baidu.dom.setBorderBoxHeight = function(element,
			height) {
		return baidu.dom.setBorderBoxSize(element, {
			height : height
		});
	};
	/*
	 ...
	 */
	baidu.dom.setOuterWidth = baidu.dom.setBorderBoxWidth = function(element,
			width) {
		return baidu.dom.setBorderBoxSize(element, {
			width : width
		});
	 ...
	 * @param {HTMLElement|string}
	 *            element 需要改变大小的元素或者元素的id.
	 * @param {Object}
	 *            [options] resizable参数配置
	...
	baidu.dom.resizable = function(element, options) {
		var target, op, resizeHandle = {}, directionHandlePosition, orgStyles = {}, range, mozUserSelect, orgCursor, offsetParent, currentEle, handlePosition, timer, isCancel = false, defaultOptions = {
			direction : [ 'e', 's', 'se' ],
			minWidth : 16,
			minHeight : 16,
			classPrefix : 'tangram',
		...
		if (!(target = baidu.dom.g(element))
				&& baidu.getStyle(target, 'position') == 'static') {
			return false;
		}
		...
		/*
		 * 必要参数的扩展 resize handle以方向命名 顺时针的顺序为 north northwest west southwest
		...
		/*
		 * {Array[Number]} rangeObject minWidth,maxWidth,minHeight,maxHeight
		 */
		range = [ op.minWidth || 0, op.maxWidth || Number.MAX_VALUE,
				op.minHeight || 0, op.maxHeight || Number.MAX_VALUE ];
		 ...
		 */
		function start(e) {
			var ele = baidu.event.getTarget(e), key = ele.key;
			currentEle = ele;
		 ...
		 * @private
		 * @param {String}
		 *            key handle的direction字符串
		 * @param {Object}
		 *            orgMousePosition 鼠标坐标{x,y}
		 * @return void
		 */
		function resize(key, orgMousePosition) {
			var xy = baidu.page.getMousePosition(), width = orgStyles['width'], height = orgStyles['height'], top = orgStyles['top'], left = orgStyles['left'], styles;

			if (key.indexOf('e') >= 0) {
				width = Math.max(
						xy.x - orgMousePosition.x + orgStyles['width'],
						range[0]);
				width = Math.min(width, range[1]);
			} else if (key.indexOf('w') >= 0) {
				width = Math.max(
						orgMousePosition.x - xy.x + orgStyles['width'],
						range[0]);
				width = Math.min(width, range[1]);
			...
			if (key.indexOf('s') >= 0) {
				height = Math.max(xy.y - orgMousePosition.y
						+ orgStyles['height'], range[2]);
				height = Math.min(height, range[3]);
			} else if (key.indexOf('n') >= 0) {
				height = Math.max(orgMousePosition.y - xy.y
						+ orgStyles['height'], range[2]);
				height = Math.min(height, range[3]);
			...
			resizeHandle['n']
					&& baidu.setStyle(resizeHandle['n'], 'width', width);
			resizeHandle['s']
					&& baidu.setStyle(resizeHandle['s'], 'width', width);
			resizeHandle['e']
					&& baidu.setStyle(resizeHandle['e'], 'height', height);
			resizeHandle['w']
					&& baidu.setStyle(resizeHandle['w'], 'height', height);

			baidu.lang.isFunction(op.onresize) && op.onresize({
				current : styles,
				original : orgStyles
			});
		}
		...
		function _getOrgStyle() {
			var offset_parent = baidu.dom.getPosition(target.offsetParent), offset_target = baidu.dom
					.getPosition(target), top, left;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/setPosition.js author: berg version: 1.0.0 date:
	 * 2010/12/14
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {object}
	 *            position 位置对象 {top: {number}, left : {number}}
	 * 
	...
	baidu.dom.setPosition = function(element, position) {
		return baidu.dom.setStyles(element,
				{
					left : position.left
							- (parseFloat(baidu.dom.getStyle(element,
									"margin-left")) || 0),
					top : position.top
							- (parseFloat(baidu.dom.getStyle(element,
									"margin-top")) || 0)
	 ...
	 * @param {HTMLElement|string}
	 *            element 目标元素或目标元素的id
	 * @remark 注意1：如果在CSS中定义此元素的样式为display:none
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/toggle.js author: allstar version: 1.1.0 date: 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/dom/toggleClass.js author: berg version: 1.0 date: 2010-07-06
	 */
	 ...
	 *            element 目标元素或目标元素的id
	 * @param {String}
	 *            className 指定的className。允许同时添加多个class，中间使用空白符分隔
	 * @version 1.3
	 * @remark
	 * 
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isArray.js author: erik version: 1.1.0 date: 2009/12/30
	 */
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/toArray.js author: berg version: 1.0 date: 2010-07-05
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/fn/methodize.js author: berg version: 1.0.0 date: 2010/11/02
	 */
	...
	baidu.fn.methodize = function(func, attr) {
		return function() {
			return func.apply(this, [ (attr ? this[attr] : this) ]
	...
	/**
	 * 包装函数的返回值，使其在能按照index指定的方式返回。<br/>如果其值为-1，直接返回返回值。
	 * <br/>如果其值为0，返回"返回值"的包装结果。<br/> 如果其值大于0，返回第i个位置的参数的包装结果（从1开始计数）
	 * 
	 * @author berg
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/fn/multize.js author: berg version: 1.0.0 date: 2010/11/02
	 */
	 ...
	 *            func 需要包装的函数
	 * @param {Boolean}
	 *            [recursive] 是否递归包装（如果数组里面一项仍然是数组，递归），可选
	 * @param {Boolean}
	 *            [joinArray] 将操作的结果展平后返回（如果返回的结果是数组，则将多个数组合成一个），可选
	 * @version 1.3
	 * 
	...
	baidu.fn.multize = function(func, recursive, joinArray) {
		var newFunc = function() {
			var list = arguments[0], fn = recursive ? newFunc : func, ret = [], moreArgs = [].slice
					.call(arguments, 0), i = 0, len, r;
	...
	/*
	 * Tangram Copyright 2010 Baidu Inc. All right reserved.
	 * 
	 * path: baidu/dom/element.js author: berg version: 1.0.0 date: 2010-07-12
	 */
	...
	baidu.element.Element = function(node) {
		 ...
		 * @type {Array.<Node>}
		 */
		this._dom = (node.tagName || '').toLowerCase() == 'select' ? [ node ]
				: baidu.lang.toArray(node);
	};
	...
	/*
	 * 包装静态方法，使其变成一个链条方法。 先把静态方法multize化，让其支持接受数组参数， 然后包装返回值，返回值是一个包装类
	 * 最后把静态方法methodize化，让其变成一个对象方法。
	 * 
	 * @param {Function} func 要包装的静态方法 @param {number} index 包装函数的第几个返回值
	 * 
	 * @return {function} 包装后的方法，能直接挂到Element的prototype上。 @private
	 */
	baidu.element._toChainFunction = function(func, index, joinArray) {
		return baidu.fn.methodize(baidu.fn.wrapReturnValue(baidu.fn.multize(
				func, 0, 1), baidu.element.Element, index), '_dom');
	};
	...
	baidu.element._makeChain = function() { // 将dom/event包下的东西挂到prototype里面
		var proto = baidu.element.Element.prototype, fnTransformer = baidu.element._toChainFunction;

		// 返回值是第一个参数的包装
		baidu.each(("draggable droppable resizable").split(' '), function(fn) {
		...
		baidu
				.each(
						...
						function(fn) {
							proto[fn] = proto[fn
									.replace(/^get[A-Z]/g, stripGet)] = fnTransformer(
									baidu.dom[fn], -1);
						});
		...
		baidu
				.each(
						("addClass empty hide show insertAfter insertBefore insertHTML removeClass "
								...
								+ "setBorderBoxWidth setBorderBoxHeight setPosition children query")
								.split(' '),
						function(fn) {
							proto[fn] = proto[fn
									.replace(/^get[A-Z]/g, stripGet)] = fnTransformer(
		...
		proto['q'] = proto['Q'] = fnTransformer(function(arg1, arg2) {
			return baidu.dom.q.apply(this, [ arg2, arg1 ].concat([].slice.call(
					arguments, 2)));
		}, 0);
		 ...
		 *            fn 事件触发时要调用的方法
		 * @version 1.3
		 * @remark 包装event的快捷方式具体包括blur、focus、focusin、focusout、load 、resize
		...
		/**
		 * 把get去掉 链里面的方法可以不以get开头调用 如 baidu.element("myDiv").parent() ==
		 * baidu.element("myDiv").getParent(); TODO: 合并getter和setter.
		 * baidu.e('myDiv').style() & baidu.e('myDiv').style('width', '100');
		 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/element/extend.js author: berg version: 1.0.0 date:
	 * 2010/12/16
	 */
	...
	baidu.element.extend = function(json) {
		var e = baidu.element;
		baidu.object.each(json,
				function(item, key) {
					e.Element.prototype[key] = baidu.element._toChainFunction(
							item, -1);
				});
	};
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/EventArg.js author: erik version: 1.1.0 date:
	 * 2010/01/11
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/_eventFilter.js author: rocy version: 1.0.0 date:
	 * 2010/10/29
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/_eventFilter/_crossElementBoundary.js author: Rocy,
	 * berg version: 1.0.0 date: 2010/12/16
	 */
	...
	baidu.event._eventFilter._crossElementBoundary = function(listener, e) {
		var related = e.relatedTarget, current = e.currentTarget;
		if (related === false ||
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/fn/bind.js author: berg version: 1.0.0 date: 2010/11/02
	 */
	 ...
	 * @param {Function|String}
	 *            handler 要绑定的函数，或者一个在作用域下可用的函数名
	 * @param {Object}
	 *            obj 执行运行时this，如果不传入则运行时this为函数本身
	 * @param {args*
	 *            0..n} args 函数执行时附加到执行时函数前面的参数
	 * @version 1.3
	 * 
	...
	baidu.fn.bind = function(func, scope) {
		...
		return function() {
			var fn = baidu.lang.isString(func) ? scope[func] : func, args = (xargs) ? xargs
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/_eventFilter/mouseenter.js author: Rocy version: 1.0.0
	 * date: 2010/11/09
	 */

	/**
	 * 用于为非IE浏览器添加mouseenter的支持; mouseenter事件仅在鼠标进入元素区域触发一次,
	 * 当鼠标在元素内部移动的时候不会多次触发.
	 */
	baidu.event._eventFilter.mouseenter = window.attachEvent ? null : function(
		...
		return {
			...
			listener : baidu.fn.bind(
					baidu.event._eventFilter._crossElementBoundary, this,
					listener)
		}
	};
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/_eventFilter/mouseleave.js author: Rocy, berg version:
	 * 1.0.0 date: 2010/11/09
	 */

	/**
	 * 用于为非IE浏览器添加mouseleave的支持; mouseleave事件仅在鼠标移出元素区域触发一次,
	 * 当鼠标在元素区域内部移动的时候不会触发.
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/_unload.js author: erik, berg version: 1.1.0 date:
	 * 2009/12/16
	 */
	...
	baidu.event._unload = function() {
		var lis = baidu.event._listeners, len = lis.length, standard = !!window.removeEventListener, item, el;
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/object/values.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/fire.js author: linlingyu version: 1.1.0 date:
	 * 2010/10/28
	 */
	 ...
	 * 
	 * @param {Boolean}
	 *            options.bubbles 是否冒泡
	 * @param {Boolean}
	 *            options.cancelable 是否可以阻止事件的默认操作
	 * @param {window|null}
	 *            options.view 指定 Event 的 AbstractView
	 * @param {1|Number}
	 *            options.detail 指定 Event 的鼠标单击量
	 * @param {Number}
	 *            options.screenX 指定 Event 的屏幕 x 坐标
	 * @param {Number}
	 *            options.screenY number 指定 Event 的屏幕 y 坐标
	 * @param {Number}
	 *            options.clientX 指定 Event 的客户端 x 坐标
	 * @param {Number}
	 *            options.clientY 指定 Event 的客户端 y 坐标
	 * @param {Boolean}
	 *            options.ctrlKey 指定是否在 Event 期间按下 ctrl 键
	 * @param {Boolean}
	 *            options.altKey 指定是否在 Event 期间按下 alt 键
	 * @param {Boolean}
	 *            options.shiftKey 指定是否在 Event 期间按下 shift 键
	 * @param {Boolean}
	 *            options.metaKey 指定是否在 Event 期间按下 meta 键
	 * @param {Number}
	 *            options.button 指定 Event 的鼠标按键
	 * @param {Number}
	 *            options.keyCode 指定 Event 的键盘按键
	 * @param {Number}
	 *            options.charCode 指定 Event 的字符编码
	 * @param {HTMLElement}
	 *            options.relatedTarget 指定 Event 的相关 EventTarget
	 * @version 1.3
	 * 
	 * @returns {HTMLElement} 目标元素
	 */
	(function() {
		var browser = baidu.browser, keys = {
			keydown : 1,
			keyup : 1,
			keypress : 1
		}, mouses = {
			...
			"KeyEvents" : [ "bubbles", "cancelable", "view", "ctrlKey",
		...
		function eventsHelper(type, eventType, options) {// 非IE内核的事件辅助
			...
			var param = baidu.object.values(parse(parameters[eventType],
					options)), evnt = document.createEvent(eventType);
			param.unshift(type);
		...
		function keyEvents(type, options) {// keyEvents
			...
			if (document.createEvent) {
				...
				options.keyCode = options.charCode > 0 ? options.charCode
						: options.keyCode;
		...
		function mouseEvents(type, options) {// mouseEvents
			...
			if (document.createEvent) {
				...
				options.button = options.button == 0 ? 1
						: options.button == 1 ? 4 : baidu.lang
								.isNumber(options.button) ? options.button : 0;
				evnt = eventObject(options);
			}
			return evnt;
		}
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/get.js author: erik version: 1.1.0 date: 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/getKeyCode.js author: erik version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/getPageX.js author: erik version: 1.1.0 date:
	 * 2009/12/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/getPageY.js author: erik version: 1.1.0 date:
	 * 2009/12/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/once.js author: wangcheng version: 1.1.0 date:
	 * 2010/10/29
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/stopPropagation.js author: erik version: 1.1.0 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/event/stop.js author: erik version: 1.1.0 date: 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2011 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/fn/abstractMethod.js author: leeight version: 1.0.0 date:
	 * 2011/04/29
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/json.js author: erik version: 1.1.0 date: 2009/12/02
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/json/parse.js author: erik, berg version: 1.2 date:
	 * 2009/11/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/json/decode.js author: erik, cat version: 1.3.4 date:
	 * 2010/12/23
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/json/stringify.js author: erik version: 1.1.0 date:
	 * 2010/01/11
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/json/encode.js author: erik, cat version: 1.3.4 date:
	 * 2010/12/23
	 */
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/Class/addEventListeners.js author: berg version: 1.0
	 * date: 2010-07-05
	 */
	 ...
	 * @param {object}
	 *            events json对象，key为事件名称，value为事件被触发时应该调用的回调函数
	 * @param {Function}
	 *            fn 要挂载的函数
	 * @version 1.3
	 */
	/*
	 * addEventListeners("onmyevent,onmyotherevent", fn); addEventListeners({
	 * "onmyevent" : fn, "onmyotherevent" : fn1 });
	 */
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * @author: meizz @namespace: baidu.lang.createClass @version: 2010-05-13
	 */
	...
	baidu.lang.createClass = function(constructor, options) {
		...
		typeof options.className == "string"
				&& (fp._className = options.className);
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/decontrol.js author: meizz version: 1.1.0 $date$
	 */
	 ...
	 * @author rocy
	 */
	baidu.lang.eventCenter = baidu.lang.eventCenter
	 ...
	 * tangram copyright 2011 baidu inc. all rights reserved.
	 * 
	 * path: baidu/lang/getModule.js author: leeight version: 1.1.0 date:
	 * 2011/04/29
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/inherits.js author: erik version: 1.1.0 date: 2009/11/24
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/instance.js author: meizz, erik version: 1.1.0 date:
	 * 2009/12/1
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isBoolean.js author: berg version: 1.0.0 date:
	 * 2010/10/12
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isDate.js author: berg version: 1.0.0 date: 2010/10/12
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isElement.js author: erik version: 1.1.0 date:
	 * 2009/12/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/lang/isObject.js author: erik version: 1.1.0 date: 2009/12/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/number/comma.js author: dron, erik, berg version: 1.2.0 date:
	 * 2010/09/07
	 */
	...
	baidu.number.comma = function(source, length) {
		...
		source[0] = source[0].replace(new RegExp('(\\d)(?=(\\d{' + length
				+ '})+$)', 'ig'), "$1,");
		return source.join(".");
	};
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/number/randomInt.js author: berg version: 1.0.0 date:
	 * 2010/12/14
	 */
	 ...
	 * @param {Object}
	 *            source 需要进行拷贝的对象
	 * @remark 对于Object来说，只拷贝自身成员，不拷贝prototype成员
	 * @meta standard
	 * 
	...
	baidu.object.clone = function(source) {
		...
		if (!source || source instanceof Number || source instanceof String
	 ...
	 * tangram copyright 2011 baidu inc. all rights reserved.
	 * 
	 * path: baidu/object/isEmpty.js author: leeight version: 1.1.0 date:
	 * 2011/04/30
	 */

	/**
	 * 检测一个对象是否是空的. 需要注意的是： 如果污染了Object.prototype或者Array.prototype，那么
	 * baidu.object.isEmpty({})或者 baidu.object.isEmpty([])可能返回的就是false.
	 * 
	 * @param {Object}
	 *            obj 需要检测的对象.
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/object/keys.js author: erik version: 1.1.0 date: 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/object/map.js author: berg version: 1.1.0 date: 2010/12/14
	 */
	...
	/*
	 * 默认情况下，所有在源对象上的属性都会被非递归地合并到目标对象上 并且如果目标对象上已有此属性，不会被覆盖
	 */
	/**
	 * 合并源对象的属性到目标对象。
	 * 
	...
	(function() {
		var isPlainObject = function(source) {
			return baidu.lang.isObject(source)
					&& !baidu.lang.isFunction(source);
		};
		...
		function mergeItem(target, source, index, overwrite, recursive) {
			if (source.hasOwnProperty(index)) {
				if (recursive && isPlainObject(target[index])) {
					// 如果需要递归覆盖，就递归调用merge
					baidu.object.merge(target[index], source[index], {
						'overwrite' : overwrite,
						'recursive' : recursive
					});
		...
		baidu.object.merge = function(target, source, opt_options) {
			var i = 0, options = opt_options || {}, overwrite = options['overwrite'], whiteList = options['whiteList'], recursive = options['recursive'], len;

			// 只处理在白名单中的属性
			if (whiteList && whiteList.length) {
				len = whiteList.length;
				for (; i < len; ++i) {
					mergeItem(target, source, whiteList[i], overwrite,
							recursive);
				}
			} else {
				for (i in source) {
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * @author: meizz @namespace: baidu.page.createStyleSheet @version:
	 * 2010-06-12
	 */
	 ...
	 * 
	 * @param {Document}
	 *            options.document 指定在哪个document下创建，默认是当前文档
	 * @param {String}
	 *            options.url css文件的URL
	 * @param {Number}
	 *            options.index 在文档里的排序索引（注意，仅IE下有效）
	 * @version 1.2
	 * @remark ie 下返回值styleSheet的addRule方法不支持添加逗号分隔的css rule.
	 * 
	 * @see baidu.page.createStyleSheet.StyleSheet
	 * 
	 * @returns {baidu.page.createStyleSheet.StyleSheet} styleSheet对象(注意:
	 *          仅IE下,其他浏览器均返回null)
	 */
	baidu.page.createStyleSheet = function(options) {
		var op = options || {}, doc = op.document || document, s;
		...
		if (baidu.browser.ie) {
			...
			op.url
					&& (s = "<link type='text/css' rel='stylesheet' href='"
							+ op.url + "'/>");
			baidu.dom.insertHTML(doc.getElementsByTagName("HEAD")[0],
					"beforeEnd", s);
			...
			var sheet = doc.styleSheets[doc.styleSheets.length - 1], rules = sheet.rules
					|| sheet.cssRules;
			return {
				self : sheet,
	...
	/*
	 * styleSheet对象 有两个方法 addRule(selector, style, i) removeRule(i)
	 * 这两个方法已经做了浏览器兼容处理 一个集合 rules
	 */
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getHeight.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/03
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getViewHeight.js author: allstar version: 1.1.0 date:
	 * 2009/11/20
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getViewWidth.js author: allstar version: 1.1.0 date:
	 * 2009/11/20
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/getWidth.js author: allstar, erik version: 1.1.0 date:
	 * 2009/12/03
	 */
	 ...
	 *            options
	 * @param {String}
	 *            [options.className] 延迟加载的IMG的className,如果不传入该值将延迟加载所有IMG.
	 * @param {Number}
	 *            [options.preloadHeight] 预加载的高度, 可见窗口下该高度内的图片将被加载.
	 * @param {String}
	 *            [options.placeHolder] 占位图url.
	 * @param {Function}
	 *            [options.onlazyload] 延迟加载回调函数,在实际加载时触发.
	 * @author rocy
	 */
	baidu.page.lazyLoadImage = function(options) {
		...
		baidu.dom
				.ready(function() {
					var imgs = document.getElementsByTagName('IMG'), targets = imgs, len = imgs.length, i = 0, viewOffset = getLoadOffset(), srcAttr = 'data-tangram-ori-src', target;
					// 避免循环中每次都判断className
					...
					for (i = 0, len = targets.length; i < len; ++i) {
						...
						if (baidu.dom.getPosition(target).top > viewOffset) {
							...
							options.placeHolder ? target.src = options.placeHolder
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/load.js author: rocy version: 1.0.0 date: 2010/11/29
	 */
	 ...
	 * 
	 * @param {Array}
	 *            resources 资源描述数组，单个resource含如下属性.
	 * @param {String}
	 *            resources.url 链接地址.
	 * @param {String}
	 *            [resources.type] 取值["css","js","html"]，默认参考文件后缀.
	 * @param {String}
	 *            [resources.requestType]
	 *            取值["dom","ajax"]，默认js和css用dom标签，html用ajax.
	 * @param {Function}
	 *            resources.onload
	 *            当前resource加载完成的回调函数，若requestType为ajax，参数为xhr(可能失效)，responseText；若requestType为dom，无参数，执行时this为相应dom标签。.
	 * 
	 * @param {Object}
	 *            [options] 可选参数.
	 * @param {Function}
	 *            [options.onload] 资源全部加载完成的回调函数，无参数。.
	 * @param {Boolean}
	 *            [options.parallel] 是否并行加载，默认为false，串行。.
	 * @param {Boolean}
	 *            [ignoreAllLoaded] 全部加载之后不触发回调事件.主要用于内部实现.
	 * 
	 * 
	 * @remark //串行实例 baidu.page.load([ { url :
	 *         "http://img.baidu.com/js/tangram-1.3.2.js" }, {url :
	 *         "http://xxx.baidu.com/xpath/logicRequire.js", onload :
	 *         fnOnRequireLoaded }, { url :
	 *         "http://xxx.baidu.com/xpath/target.js" } ],{ onload :
	 *         fnWhenTargetOK }); //并行实例 baidu.page.load([ { url :
	 *         "http://xxx.baidu.com/xpath/template.html", onload :
	 *         fnExtractTemplate }, { url :
	 *         "http://xxx.baidu.com/xpath/style.css"}, { url :
	 *         "http://xxx.baidu.com/xpath/import.php?f=baidu.*", type : "js" }, {
	 *         url : "http://xxx.baidu.com/xpath/target.js", }, { url :
	 *         "http://xxx.baidu.com/xpath/jsonData.js", requestType : "ajax",
	 *         onload : fnExtractData } ],{ parallel : true, onload :
	 *         fnWhenEverythingIsOK });
	 */
	baidu.page.load = function(resources, options, ignoreAllLoaded) {
		// TODO failure, 整体onload能不能每个都调用; resources.charset
		options = options || {};
		var self = baidu.page.load, cache = self._cache = self._cache || {}, loadingCache = self._loadingCache = self._loadingCache
		...
		function loadByDom(res, callback) {
			...
			// CSS has no onload event on firefox and webkit platform, so hack
			// it.
			if (res.type == 'css') {
				(function() {
					// 避免重复加载
		...
		// 兼容第一个参数直接是资源地址.
		baidu.lang.isString(resources) && (resources = [ {
			url : resources
		} ]);
		...
		function loadResources(res) {
			var url = res.url, shouldContinue = !!parallel, cacheData, callback = function(
					textOrNode) {
			...
			res.requestType = res.requestType
					|| (res.type == 'html' ? 'ajax' : 'dom');
			...
			if (res.requestType.toLowerCase() == 'dom') {
				...
				baidu.ajax.get(res.url, function(xhr, responseText) {
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/loadCssFile.js author: allstar version: 1.1.0 date:
	 * 2009/11/20
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/page/loadJsFile.js author: allstar version: 1.1.0 date:
	 * 2009/11/20
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/sio.js author: erik version: 1.1.0 date: 2009/12/16
	 */
	 ...
	 * @remark 1、与callByServer不同，callback参数只支持Function类型，不支持string。
	...
	baidu.sio.callByBrowser = function(url, opt_callback, opt_options) {
		var scr = document.createElement("SCRIPT"), scriptLoaded = 0, options = opt_options
				|| {}, charset = options['charset'], callback = opt_callback
				|| function() {
	 ...
	 * @param {Function|string}
	 *            callback
	 *            服务器端调用的函数或函数名。如果没有指定本参数，将在URL中寻找options['queryField']做为callback的方法名.
	 * @param {Object}
	 *            opt_options 加载数据时的选项.
	 * @config {string} [charset] script的字符集
	 * @config {string} [queryField] 服务器端callback请求字段名，默认为callback
	 * @config {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
	 * @config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
	 * @remark 如果url中已经包含key为“options['queryField']”的query项，将会被替换成callback中参数传递或自动生成的函数名。
	 * @meta standard
	 * @see baidu.sio.callByBrowser
	 */
	baidu.sio.callByServer = function(url, callback, opt_options) {
		var scr = document.createElement('SCRIPT'), prefix = 'bd__cbs__', callbackName, callbackImpl, options = opt_options
				|| {}, charset = options['charset'], queryField = options['queryField']
				|| 'callback', timeOut = options['timeOut'] || 0, timer, reg = new RegExp(
				'(\\?|&)' + queryField + '=([^&]*)'), matches;
	...
	baidu.sio.log = function(url) {
		var img = new Image(), key = 'tangram_sio_log_'
				+ Math.floor(Math.random() * 2147483648).toString(36);
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/decodeHTML.js author: erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/encodeHTML.js author: erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/filterFormat.js author: rocy version: 1.1.2 date:
	 * 2010/06/10
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/filterFormat/escapeJs.js author: rocy version: 1.1.2
	 * date: 2010/06/12
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/filterFormat/escapeString.js author: rocy version:
	 * 1.1.2 date: 2010/06/12
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/filterFormat/toInt.js author: rocy version: 1.1.2
	 * date: 2010/06/12
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/format.js author: dron, erik version: 1.1.0 date:
	 * 2009/11/30
	 */
	 ...
	 * Tangram Copyright 2010 Baidu Inc. All rights reserved.
	 * 
	 * @author: meizz @namespace: baidu.string.formatColor @version: 2010-01-23
	 */
	...
	(function() {
		// 将正则表达式预创建，可提高效率
		var reg1 = /^\#[\da-f]{6}$/i, reg2 = /^rgb\((\d+), (\d+), (\d+)\)$/, keyword = {
			black : '#000000',
			silver : '#c0c0c0',
			gray : '#808080',
			white : '#ffffff',
		...
		baidu.string.formatColor = function(color) {
			if (reg1.test(color)) {
				...
				// 简写的颜色值: #F00
				var s1 = color.charAt(1), s2 = color.charAt(2), s3 = color
						.charAt(3);
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/getByteLength.js author: erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * tangram copyright 2011 baidu inc. all rights reserved.
	 * 
	 * path: baidu/string/stripTags.js author: leeight version: 1.1.0 date:
	 * 2011/04/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/subByte.js author: dron, erik, berg version: 1.2 date:
	 * 2010-06-30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/toHalfWidth.js author: erik version: 1.1.0 date:
	 * 2009/11/15
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/string/wbr.js author: erik version: 1.1.0 date: 2009/11/30
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/swf.js author: erik version: 1.1.0 date: 2009/11/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/swf/getMovie.js author: erik version: 1.1.0 date: 2009/11/16
	 */
	 ...
	 * @param {string}
	 *            id Flash的元素id.object标签id, embed标签name.
	 * @param {string}
	 *            property Flash的方法或者属性名称，用来检测Flash是否初始化好了.
	 * @param {Function}
	 *            loadedHandler 初始化之后的回调函数.
	...
	baidu.swf.Proxy = function(id, property, loadedHandler) {
		 ...
		 * @type {HTMLElement}
		 */
		var me = this, flash = this._flash = baidu.swf.getMovie(id), timer;
		if (!property) {
			return this;
		}
		timer = setInterval(function() {
	...
	baidu.swf.Proxy.prototype.call = function(methodName, var_args) {
		try {
			var flash = this.getFlash(), args = Array.prototype.slice
					.call(arguments);
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/swf/version.js author: erik version: 1.1.0 date: 2009/11/17
	 */
	...
	baidu.swf.version = (function() {
		...
		if (n.plugins && n.mimeTypes.length) {
			...
			if (plugin && plugin.description) {
				return plugin.description.replace(/([a-zA-Z]|\s)+/, "")
						.replace(/(\s)+r/, ".")
						+ ".0";
			}
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/swf/createHTML.js author: erik version: 1.1.0 date:
	 * 2009/11/17
	 */
	 ...
	 * @param {string}
	 *            options.scale
	 *            设置flash内容如何缩放来适应设置的宽高。允许值：showall/noborder/exactfit
	 * @param {string}
	 ...
	 *            设置swf文件中允许使用的网络API。允许值：all/internal/none
	 * @param {boolean}
	...
	baidu.swf.createHTML = function(options) {
		options = options || {};
		var version = baidu.swf.version, needVersion = options['ver']
				|| '6.0.0', vUnit1, vUnit2, i, k, len, item, tmpOpt = {}, encodeHTML = baidu.string.encodeHTML;
		...
		var vars = options['vars'], objProperties = [ 'classid', 'codebase',
				'id', 'width', 'height', 'align' ];
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/swf/create.js author: erik version: 1.1.0 date: 2009/11/17
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/url.js author: erik version: 1.1.0 date: 2009/11/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/url/escapeSymbol.js author: erik version: 1.1.0 date:
	 * 2009/11/16
	 */
	...
	baidu.url.escapeSymbol = function(source) {
		return String(source).replace(/\%/g, "%25").replace(/&/g, "%26")
				.replace(/\+/g, "%2B").replace(/\ /g, "%20").replace(/\//g,
						"%2F").replace(/\#/g, "%23").replace(/\=/g, "%3D");
	};
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/url/getQueryValue.js author: erik version: 1.1.0 date:
	 * 2009/11/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/url/jsonToQuery.js author: erik version: 1.1.0 date:
	 * 2009/11/16
	 */
	 ...
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 * 
	 * path: baidu/url/queryToJson.js author: erik version: 1.1.0 date:
	 * 2009/11/16
	 */
	...
	baidu.url.queryToJson = function(url) {
		var query = url.substr(url.lastIndexOf('?') + 1), params = query
				.split('&'), len = params.length, result = {}, i = 0, key, value, item, param;
	...
	baidu.async._isDeferred = function(obj) {
		...
		return obj && isFn(obj.success) && isFn(obj.then) && isFn(obj.fail)
				&& isFn(obj.cancel);
	};
	/*
	 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
	 ...
	 * @remark 示例: function someAsync(){ var deferred = new
	 *         baidu.async.Deferred(); setTimeout(function(){
	 *         afterSomeOperation(); if(someReason){
	 *         deferred.resolve(someValue); } else { deferred.reject(someError); }
	 *         },100); return deferred; } //用类似同步的方式调用异步操作.
	...
	baidu.async.Deferred = function() {
		...
		function fire() {
			...
			me._firing = 1;
			var chain = me._isError ? me._rejectChain : me._resolveChain, result = me._result[me._isError ? 1
					: 0];
			...
			while (chain[0] && (!me._cancelled)) {
				...
				try {
					...
					if (baidu.async._isDeferred(chainResult)) {
						...
						[].push.apply(chainResult._resolveChain,
								me._resolveChain);
						[].push
								.apply(chainResult._rejectChain,
										me._rejectChain);
		 ...
		 * 
		 * @param {Function}
		 *            onSuccess 该deferred成功时的回调函数.第一个形参为成功时结果.
		 * @param {Function}
		 *            onFail 该deferred失败时的回调函数.第一个形参为失败时结果.
		 * @return {baidu.async.Deferred} this.
		 */
	...
	/**
	 * 保证onResolve或onReject可以按序执行.
	 * 若第一个参数为deferred,则deferred完成后执行.否则立即执行onResolve,并传入第一个参数.
	 * 
	 * @param {baidu.async.Deferred|*}
	 *            deferredOrValue deferred实例或任意值.
	 * @param {Function}
	 *            onResolve 成功时的回调函数.若第一个参数不是Deferred实例,则立即执行此方法.
	 * @param {Function}
	 *            onReject 失败时的回调函数.
	 * @remark 示例一:异步调用: baidu.async.when(asyncLoad(), onResolve,
	 *         onReject).then(nextSuccess, nextFail); 示例二:同步异步不确定的调用:
	 *         baidu.async.when(syncOrNot(), onResolve,
	 *         onReject).then(nextSuccess, nextFail); 示例三:同步接异步的调用:
	 *         baidu.async.when(sync(), onResolve, onReject).then(asyncSuccess,
	 *         asyncFail).then(afterAllSuccess, afterAllFail);
